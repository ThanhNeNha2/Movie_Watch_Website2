...
const SearchName = () => {
  ...
  // Map keyword to display title
  const displayTitle = keyword
    ? `Kết quả tìm kiếm: ${decodeURIComponent(keyword)}`
    : "Kết quả tìm kiếm";
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["movies", keyword, filters.page],
    queryFn: fetchMovies,
  ...
  const pagination = data?.pageProps.data.params.pagination || {
    totalItems: 0,
    totalItemsPerPage: 24,
    currentPage: 1,
    pageRanges: 3, // Updated to match API default
  };
