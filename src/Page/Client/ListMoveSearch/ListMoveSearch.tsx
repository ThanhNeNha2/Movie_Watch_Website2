...
const ListMoveSearch = () => {
  ...
  const filters = {
    ...
    type: searchParams.get("type") || "phim-vietsub", // Default to phim-vietsub
  };
  ...
  const fetchMovies = async (): Promise<Movie[]> => {
    const response = await axios.get("http://localhost:8080/api/list-movie", {
      params: {
        ...
        category: filters.category, // Match backend parameter
        country: filters.country, // Match backend parameter
        year: filters.year,
        type: filters.type, // Include type parameter
      },
    });
